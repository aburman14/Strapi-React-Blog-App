    ...
    < div className='flex flex-column'>
    {
      allposts.map((post,id)=>{
        ...
        let logged_in=JSON.parse(localStorage.getItem('creds')).user.username        
          return (
            <main key={id} className='my-2 mx-10 max-w-md'>
              <div >
                <h1 className='text-4xl capitalize my-2'>{post.attributes.title}</h1>
                <div className='flex flex-column justify-between	'>
                <p>By {posted_by}</p>
                <p>{post.attributes.post_created}</p>
                <div className='flex flex-row gap-4'>
                { logged_in === posted_by ? 
                <span onClick={()=>edit(post.id)} className='cursor-pointer'><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
                ...
                <img src={img_url} alt="test" className='rounded-md object-cover my-2' />
